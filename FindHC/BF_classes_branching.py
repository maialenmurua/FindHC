...
class BF_Algorithms:
    ...
    def Branch_and_Fix(self):
        ...
        if self.ind_degree2=='Yes':
            ...
            if feasible_solution==0: # This means it is not possible to find a HC with the current graph
                ...
                self.__init__(adj_mat,self.beta,self.level,self.branching_method,self.ind_degree2,self.solver) 
	...
	# Obtain the solution with the First LP
        ...
        self.Get_Sol(x) # Get the policy from x
        ...
        if splitting_node==-1: # An HC was found return code=1   
            ...
            return 1, self.hg,self.level,calls
             ...
             found=False
             i=0
             while (found==False) & (i<len(mylist)): # Explore the branches of the node of the tree while 
                                                     # a solution was found or all the branches have been explored
            ...
            # and constraints and apply the Branch and Fix algorithm on it 
                ...
                code,hg,level,calls = NewBF.Branch_and_Fix() # Call recursively the algorithm
                
                calls+=1
                found=(code==1)
                i=i+1

             if found==True: # Stop the execution when a solution was found
                 return 1, hg,self.level,calls
             else: # When exploring all the branches a solution was not found exit. 
                 return -1, [],self.level,calls
