...
class BF_Algorithms:
    def __init__(self,A,beta,lev,branching_method,graph_n,perm,ind_degree2,solver):
        self.graph=HGraph(A)
        self.N = self.graph.N
        self.mu=1/(2*self.N) 
        self.A=self.graph.A
    ...
    def Branch_and_Fix(self):
        ...
        if self.ind_degree2=='Yes':
            ...
            if feasible_solution==0: # This means it is not possible to find a HC with the current graph
                ...
                self.__init__(adj_mat,self.beta,self.level,self.branching_method,self.graph_n,self.perm,self.ind_degree2,self.solver) # Redefined the graph with the new matrix

            self.fixed_arcs=fixed_arcs  #
        
        elif self.ind_degree2=='No':
            self.__init__(self.A,self.beta,self.level,self.branching_method,self.graph_n,self.perm,self.ind_degree2,self.solver)
	...
	# Obtain the solution with the First LP
        ...
        if splitting_node==-1: # An HC was found return code=1   
            ...
            # and constraints and apply the Branch and Fix algorithm on it 
                ...
                NewBF = BF_Algorithms(self.A, self.beta,self.level+1,self.branching_method,self.graph_n,self.perm,self.ind_degree2,self.solver)
                ...
                if graph_is_circuit==True:
                    ...
                    reduced_BF = BF_Algorithms(reduced_mat,self.beta,self.level+1,self.branching_method,self.graph_n,self.perm,self.ind_degree2,self.solver)
